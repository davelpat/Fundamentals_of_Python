"""
Instructions for programming Exercise 9.4
...
after a user has had three successive failures. The program should also disable
the login button when this happens.
"""
...
class ATM(EasyFrame):
    ...
    def login(self):
        ...
        if self.account:
            ...
            self.login_tries += 1
            if self.login_tries == ATM.MAX_FAILED_LOGINS:
                self.loginButton["state"] = "disabled"
                self.statusField.setText("Calling Police")
            else:
